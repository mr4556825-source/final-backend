...
// تأكد من استيراد الموديلات الصحيحة التي تحمل اسم Volleyball
const VolleyballSettings = require("../models/VolleyballSettings");
const User = require("../models/user"); 
const VolleyballRegistration = require("../models/VolleyballRegistration"); 
// تأكد أن المسار هنا يشير إلى موديل الماتشات الأساسي (Match) أو موديل الكرة الطائرة (VolleyballMatch)
const Match = require("../models/VolleyballMatch"); 
const VolleyballLeaderboard = require('../models/VolleyballLeaderboard');

// 1. Middleware للتحقق من حالة التسجيل
async function checkRegistration(req, res, next) {
  let settings = await VolleyballSettings.findOne();
  if (!settings) settings = await VolleyballSettings.create({});
...
// 3. إضافة لاعب (تسجيل في رياضة الكرة الطائرة)
router.post("/add", checkRegistration, async (req, res) => {
  try {
    ...
    const existing = await VolleyballRegistration.findOne({ userId: user._id });
    if (existing) return res.status(409).json({ message: "Already registered" });

    const reg = new VolleyballRegistration({ userId: user._id, name: user.name, team });
    await reg.save();
...
router.get("/", async (req, res) => {
  const players = await VolleyballRegistration.find({});
  res.json(players);
});

// 5. مسح لاعب
router.delete("/delete/:id", async (req, res) => {
  await VolleyballRegistration.findByIdAndDelete(req.params.id);
...
router.post("/generate-draw", async (req, res) => {
  try {
    ...
    await VolleyballLeaderboard.deleteMany({}); // مسح الترتيب القديم للبدء من جديد
...
// 9. تحديث النتيجة وحساب النقاط تلقائياً
router.post("/update-score", async (req, res) => {
  ...
  try {
    ...
    for (let tName of teamNames) {
      ...
      await VolleyballLeaderboard.findOneAndUpdate(
        { team: tName },
        stats,
...
router.get("/leaderboard", async (req, res) => {
  const leaderboard = await VolleyballLeaderboard.find().sort({ points: -1, goalsFor: -1 });
