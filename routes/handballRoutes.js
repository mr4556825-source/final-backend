...
// تأكد من استيراد الموديلات الصحيحة التي تحمل اسم Handball
const HandballSettings = require("../models/HandballSettings");
const User = require("../models/user"); 
const HandballRegistration = require("../models/HandballRegistration"); 
// تأكد أن المسار هنا يشير إلى موديل الماتشات الأساسي (Match) أو موديل الهاندبول (HandballMatch)
const Match = require("../models/HandballMatch"); 
const HandballLeaderboard = require('../models/HandballLeaderboard');

// 1. Middleware للتحقق من حالة التسجيل
async function checkRegistration(req, res, next) {
  let settings = await HandballSettings.findOne();
  if (!settings) settings = await HandballSettings.create({});
...
// 3. إضافة لاعب (تسجيل في رياضة كرة اليد)
router.post("/add", checkRegistration, async (req, res) => {
  try {
    ...
    const existing = await HandballRegistration.findOne({ userId: user._id });
    if (existing) return res.status(409).json({ message: "Already registered" });

    const reg = new HandballRegistration({ userId: user._id, name: user.name, team });
    await reg.save();
...
router.get("/", async (req, res) => {
  const players = await HandballRegistration.find({});
  res.json(players);
});

// 5. مسح لاعب
router.delete("/delete/:id", async (req, res) => {
  await HandballRegistration.findByIdAndDelete(req.params.id);
...
router.post("/generate-draw", async (req, res) => {
  try {
    ...
    await HandballLeaderboard.deleteMany({}); // مسح الترتيب القديم للبدء من جديد
...
router.post("/update-score", async (req, res) => {
  ...
  try {
    ...
    for (let tName of teamNames) {
      ...
      await HandballLeaderboard.findOneAndUpdate(
        { team: tName },
        stats,
...
router.get("/leaderboard", async (req, res) => {
  const leaderboard = await HandballLeaderboard.find().sort({ points: -1, goalsFor: -1 });
